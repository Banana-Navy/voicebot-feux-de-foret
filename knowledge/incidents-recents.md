# Retours d'expérience récents — feux de forêt et de végétation

Version vérifiée le 17 août 2026. Ces événements sont des cas historiques destinés à améliorer les réponses. Ils ne décrivent jamais une situation en cours. Le bot doit toujours consulter une source locale récente avant d'affirmer qu'une route, un massif ou une zone est fermé ou évacué.

## Belgique — Lagland et Han-sur-Lesse, 18 mars 2025

Le SPW indique qu'un feu de broussailles a détruit environ 8 à 10 hectares dans le camp militaire de Lagland à Arlon, et qu'un autre incendie a ravagé près de 3 hectares de végétation et de bois à Han-sur-Lesse. La communication régionale a souligné la sécheresse des herbes mortes et des rafales dépassant localement 50 km/h.

Enseignements pour le voicebot :

- demander un lieu précis, des repères et le meilleur accès ;
- distinguer feu établi, fumée seule et départ de feu ;
- demander si des personnes sont exposées, sans retarder la consigne 112 ;
- ne pas conseiller d'éteindre un feu établi ;
- rappeler que le vent rend l'évolution imprévisible, sans la prédire.

Source : https://environnement.wallonie.be/home/a-la-une/actualites/actualites/drapeau-rouge-dans-les-hautes-fagnes-acces-interdit-en-raison-du-risque-d-incendie.html

## Belgique — Hautes Fagnes, mai 2025

Le Département de la Nature et des Forêts a hissé le drapeau rouge et interdit l'accès à des zones sensibles. Des panneaux ont été installés aux entrées des chemins concernés et une carte des chemins interdits a été publiée. La fermeture devait rester en vigueur jusqu'à l'amélioration des conditions.

Enseignements pour le voicebot :

- une interdiction est dynamique : annoncer l'autorité, la zone et l'heure de vérification ;
- orienter vers la carte ou le site officiel de la zone ;
- ne jamais déduire qu'un chemin voisin est ouvert ;
- expliquer la signification du drapeau rouge et demander de respecter la signalétique sur place.

Source : https://environnement.wallonie.be/home/a-la-une/actualites/actualites/drapeau-rouge-dans-les-hautes-fagnes-acces-interdit-en-raison-du-risque-d-incendie.html

## Belgique — vigilance wallonne, avril et juillet 2026

En avril 2026, la surveillance des agents du DNF a été renforcée, les gestionnaires de réserves informés et certaines cartes de chemins fermés publiées. En juillet 2026, le risque a été annoncé élevé en forêt et très élevé dans les milieux ouverts, particulièrement dans les provinces de Liège, Luxembourg et Namur.

Enseignements pour le voicebot :

- expliquer que risque élevé ne signifie pas qu'un feu est en cours ;
- distinguer le niveau régional des décisions d'un bourgmestre, gouverneur ou gestionnaire de site ;
- rediriger vers la commune ou le domaine naturel pour les interdictions locales ;
- dater chaque niveau de vigilance et ne jamais réutiliser une ancienne alerte.

Sources :

- https://environnement.wallonie.be/actualite/risque-accru-d-incendie-en-milieux-naturels-vigilance-renforcee-cette-semaine-en-wallonie
- https://environnement.wallonie.be/actualite/risque-d-incendie-eleve-a-tres-eleve-appel-a-la-vigilance-en-wallonie

## Crète, Grèce — juillet 2025

Un feu rapidement propagé par des vents forts a menacé plusieurs localités près d'Ierapetra. Les habitants ont reçu un message 112 diffusé par cellule pour évacuer. Les autorités ont engagé 155 pompiers, 47 véhicules, des équipes terrestres et sept hélicoptères. Copernicus EMS Rapid Mapping a été activé pour cartographier l'étendue du feu.

Enseignements pour le voicebot :

- un message d'évacuation doit préciser la zone concernée et la destination ;
- le bot doit répéter fidèlement le message officiel sans ajouter d'itinéraire ;
- les données cartographiques soutiennent les autorités, mais ne constituent pas seules un ordre au public ;
- en cas de messages multiples, privilégier le plus récent et annoncer son heure.

Source Commission européenne/JRC : https://data.jrc.ec.europa.eu/dataset/9d8d35b6-0b6a-5810-bff8-1d6c0a0b0c37

## Espagne — été 2025

La Commission européenne rapporte plus de 130 grands feux, presque 300 routes fermées et plus de 35 900 personnes évacuées. L'Espagne a activé le mécanisme européen de protection civile ; des équipes et véhicules de plusieurs pays ont été déployés.

Enseignements pour le voicebot :

- ne jamais proposer un itinéraire à partir d'informations générales ;
- orienter vers la police, la commune ou le service routier officiel pour les routes ouvertes ;
- prévoir des réponses aux questions sur les centres d'accueil, les animaux, les médicaments et la mobilité ;
- rester utile lors de crises simultanées en donnant une action courte et territorialisée.

Source Commission européenne : https://civil-protection-humanitarian-aid.ec.europa.eu/what/civil-protection/european-civil-protection-pool_en

## Chios, Grèce — juin 2025

L'OMS Europe rapporte qu'un état d'urgence et plusieurs ordres d'évacuation ont concerné jusqu'à vingt localités. L'événement a mis en évidence les effets de la fumée, de la chaleur et du stress, ainsi que la nécessité d'aider les enfants, personnes âgées, personnes enceintes et malades chroniques.

Enseignements pour le voicebot :

- inclure systématiquement les besoins des personnes vulnérables ;
- conseiller de suivre les ordres locaux et de réduire l'exposition à la fumée ;
- reconnaître les symptômes nécessitant une aide médicale ;
- répondre aussi aux questions d'après-crise et de santé mentale sans poser de diagnostic.

Source OMS Europe : https://www.who.int/europe/news-room/photo-stories/item/chios-on-fire--one-photographer-s-view-of-a-growing-health-threat

## Principes de communication issus des cas

1. Commencer par l'action utile, puis expliquer.
2. Dire ce qui est confirmé, ce qui ne l'est pas et la source consultée.
3. Dater toute information opérationnelle.
4. Ne jamais confondre vigilance, interdiction, mise à l'abri et évacuation.
5. Répéter fidèlement les zones, destinations et horaires d'un ordre officiel.
6. Prévoir des réponses pour les personnes sans voiture, avec handicap, enfants, animaux ou traitements médicaux.
7. Utiliser plusieurs canaux : BE-Alert, autorités locales, médias, sites officiels et messages téléphoniques.
